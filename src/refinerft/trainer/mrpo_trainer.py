...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if reward_weights is not None:
            ...
            self.reward_weights = torch.tensor(custom_args.reward_weights, dtype=torch.float32)
        else:
            self.reward_weights = torch.ones(len(reward_funcs), dtype=torch.float32)

        self.embedding_model = None
        # self.embedding_tokenizer = None
        if custom_args is not None and hasattr(custom_args, 'reward_funcs') and 'embedding_similarity' in custom_args.reward_funcs:
            # from transformers import AutoTokenizer, AutoModel
            from sentence_transformers import SentenceTransformer
            model_name = custom_args.embeding_model_name
        ...
        if self.embedding_model is not None:
            if self.is_deepspeed_enabled:
                self.embedding_model = prepare_deepspeed(self.embedding_model, self.accelerator)
            else:
                self.embedding_model = self.accelerator.prepare_model(self.embedding_model, evaluation_mode=True)
    ...
    def training_step(
        ...
        if hasattr(self, "reward_weights_optimizer"):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # completions[random_index][0]['content'] = reference
        #-----------------------------------------------------------------
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                if self.embedding_model is not None:
                    reward_kwargs['embedding_model'] = self.embedding_model
                    reward_kwargs['embedding_tokenizer'] = self.embedding_tokenizer if hasattr(self, 'embedding_tokenizer') else None
        ...
        advantages = (advantages * self.reward_weights.to(device)).sum(dim=1)  # (B*G,)
    ...
    def evaluation_loop(
        ...
        # COCO evaluation: run subprocess when dataset is COCO (does not use dataloader)
        is_coco_dataset = (
            COCO_EVALUATION_AVAILABLE
            and self.custom_args is not None
            and dataset_name_mapping.get(self.custom_args.dataset_name) == "coco"
        )
        if is_coco_dataset and self.accelerator.is_main_process:
            try:
                # Resolve checkpoint path (trainer saves to output_dir/checkpoint-{step} before eval)
                ckpt_path = os.path.join(self.args.output_dir, f"checkpoint-{self.state.global_step}")
                if not os.path.exists(ckpt_path):
                    ckpt_path = self.args.output_dir
                model_base = getattr(self.custom_args, "model", "Qwen/Qwen2-VL-2B-Instruct")
                model_cache_dir = getattr(self.custom_args, "model_cache_dir", "./share_models")
                coco_data_path = getattr(self.custom_args, "coco_data_path", "./data/coco")

                # Resolve coco_evaluation directory (relative to project root)
                project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
                coco_eval_dir = os.path.join(project_root, "coco_evaluation")
                if not os.path.exists(coco_eval_dir):
                    coco_eval_dir = os.path.join(os.getcwd(), "coco_evaluation")

                start_time = time.time()
                result = subprocess.run(
                    [
                        "python", "Qwen2_VL_coco_infere.py",
                        "--model_path", os.path.abspath(ckpt_path),
                ...
                print(f"COCO subprocess evaluation time: {(time.time() - start_time) / 60:.2f} minutes")

                if result.stderr:
                    key_value_pairs = re.findall(r"Key: (.*), Value: ([\d.]+)", result.stderr)
                    for key, value in key_value_pairs:
                        eval_results[f"{metric_key_prefix}_{key.strip()}"] = float(value)
                    if "mAP for selected categories:" in result.stderr:
                        try:
                            ...
                            eval_results[f"{metric_key_prefix}_mAP"] = round(float(nums[0]) / 100, 3)
                    ...
                    if "Avg completion length (tokens):" in result.stderr:
                        try:
                            ...
                            if avg_len_match:
                                eval_results[f"{metric_key_prefix}_avg_completion_length"] = round(float(avg_len_match.group(1)), 2)
                        except (IndexError, ValueError):
                            pass

                if eval_results:
                    print(f"COCO Evaluation results: {eval_results}")
            except Exception as e:
                import traceback
                traceback.print_exc()
        ...
        if is_coco_dataset:
            # Broadcast eval_results from main process to all processes
            eval_results_list = gather_object([eval_results])
            eval_results = eval_results_list[0] if eval_results_list else {}
            eval_results["eval_accuracy"] = eval_results["mAP"] if "mAP" in eval_results else 0
