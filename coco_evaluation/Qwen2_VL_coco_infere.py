...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, default="/research/cvl-zhujie4/Visual-RFT/share_models/coco-8cate-4shot-grpo_0917_1737")
    parser.add_argument("--model_base", type=str, default="Qwen/Qwen2-VL-2B-Instruct")
    parser.add_argument("--cache_dir", type=str, default="/research/cvlshare/cvl-zhujie4/Refine-RFT/share_models")
    parser.add_argument("--val_type", type=str, default="few", choices=["few", "open"])
    parser.add_argument("--coco_data_path", type=str, default="/research/cvl-zhujie4/data/coco")
...
def run(rank, 
    ...
    if os.path.exists(os.path.join(model_path, "adapter_config.json")):
        ...
        logger.info(f"Load lora ckpt from {model_path}")
    else:
        ...
        logger.info(f"Load fully finetuned ckpt from {model_path}")
    ...
    with open(os.path.join(coco_data_path, 'annotations_trainval2017/annotations/instances_val2017.json'), 'r') as json_file:
    ...
    bbox_count = 0
    pred_results = []
    completion_lengths = []  # Track token count for each generated response
    if '2B' in model_path or '2B' in model_base:
        exist_cat = json.load(open("./exist_map_coco_Qwen2_vl_2B_baseline.json", 'r'))
    elif '7B' in model_path or '7B' in model_base:
        exist_cat = json.load(open("./exist_map_coco_Qwen2_vl_7B_baseline.json", 'r'))
    else:
        raise ValueError(f"Could not find the exist map!")
      
    for image in tqdm(split_images): 
        image_id = image['id']
        image_height = image['height']
        image_width = image['width']
        image_path = os.path.join(coco_data_path, 'val2017/val2017', image['file_name'])    ### Modify according to your own image path.
        ...
        for cate in exist_cat[str(image_id)]:
            ...
            # logger.info(f"Rank {rank} Processing image {image_id} with category {category}")
            ...
            try:
                ...
                generated_ids = model.generate(**inputs, max_new_tokens=1024, use_cache=True, temperature=0.0, do_sample=False) # Use greedy decoding for evaluation
                ...
                response = response[0]
                # Record completion length (number of generated tokens) for this response
                completion_lengths.append(len(generated_ids_trimmed[0]))
                ...
                try:
                    ...
                    error_count+=1
                    logger.info('Can not extract bbox, Error number: ' + str(error_count)) 
            except Exception as e:
                    error_count+=1
                    logger.info(e)
                    logger.info('Can not extract answer, Error number: ' + str(error_count)) 
    return [error_count, pred_results, completion_lengths]

def main(model_path, 
         model_base="Qwen/Qwen2-VL-2B-Instruct", 
         cache_dir="/research/cvl-zhujie4/Visual-RFT/share_models", 
         val_type="few",
         coco_data_path='./data/coco/'):
    ...
    if multiprocess:
        ...
        global_results = []
        global_completion_lengths = []
        for i in range(world_size):
            global_count_error += int(result_lists[i][0])
            global_results = global_results + result_lists[i][1]
            global_completion_lengths.extend(result_lists[i][2])

        logger.info('Error number: ' + str(global_count_error))  
        ### save path
        save_path = f'./results/prediction_results_{model_path.split("/")[-1]}_{val_type}.json'
        ...
        # evaluate mAP and AP for all categories
        evaluator = CocoDetectionEvaluator(os.path.join(coco_data_path, 'annotations_trainval2017/annotations/instances_val2017.json'))
        results, per_class_results = evaluator.evaluate(save_path, './results')
        if val_type == "few":
            selected_cate = ['bus', 'train', 'fire hydrant', 'stop sign', 'cat', 'dog', 'bed', 'toilet']
        elif val_type == "open":
            selected_cate = ['mouse', 'fork', 'hot dog', 'cat', 'airplane', 'suitcase', 'parking meter', 'sandwich', 'train', 'hair drier', 'toilet', 'toaster', 'snowboard', 'frisbee', 'bear']
        ...
        for item in per_class_results:
            for key, value in item.items():
                if key in selected_cate:
                    logger.info(f"Key: {key}, Value: {value:.2f}")
                    AP_sum += value
        logger.info(f"mAP for selected categories: {(AP_sum)/(len(selected_cate)):.2f}")

        # Output average completion length (tokens per response)
        avg_completion_length = np.mean(global_completion_lengths) if global_completion_lengths else 0.0
        logger.info(f"Avg completion length (tokens): {avg_completion_length:.2f}")
...
if __name__ == "__main__":
    args = parse_args()
    main(args.model_path, args.model_base, args.cache_dir, args.val_type, args.coco_data_path)
